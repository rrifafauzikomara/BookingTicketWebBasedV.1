    		...
    		<form action="" method="post"> 
    			...
    			<input type="text" name="alamat" class="lg" placeholder="Alamat Lengkap"> 
    			 <p class="contact"><label for="phone">No HP</label></p> 
           		<input type="text" name="no_hp" class="lg" placeholder="Masukan No HP">
...
<?php
    ...
    if (@$_POST['register']) {
        ...
        $password = @$_POST['password'];
            ...
            if ($nama_user == '' || $alamat == '' || $no_hp == '' || $email == '' || $username == '' || $password == '') {
                    ?> <script type="text/javascript">alert("Inputan tidak boleh kosong !!");</script> <?php
                } else {
                    $sql_insert = "insert into tb_registrasi values ('', '$nama_user', '$alamat', '$no_hp', '$email', '$username', '$password','user')" or die (mysqli_error());
